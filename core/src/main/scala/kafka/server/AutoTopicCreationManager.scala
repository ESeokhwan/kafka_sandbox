  ...
  private def creatableTopic(topic: String): CreatableTopic = {
    topic match {
      case GLOBAL_SEQUENCE_INDEX_TOPIC_NAME =>
        new CreatableTopic()
          .setName(topic)
          .setNumPartitions(config.globalSequenceCoordinatorConfig.indexTopicNumPartitions())
          .setReplicationFactor(config.globalSequenceCoordinatorConfig.indexTopicReplicationFactor())
          .setConfigs(convertToTopicConfigCollections(config.globalSequenceCoordinatorConfig.indexTopicConfigs()))
