 ...
 */
public class TransientTopicRecordAccumulator {
    ...
    private final ConcurrentMap<String, Deque<ProducerBatch>> transientTopicInfoMap = new CopyOnWriteMap<>();
     ...
     */
    public TransientTopicRecordAccumulator(LogContext logContext,
                                           int batchSize,
        ...
        this.log = logContext.logger(TransientTopicRecordAccumulator.class);
        this.closed = false;
     ...
     * <p>
     *
     * @param ttName The name of transient topic to which this record is being sent
     ...
     */
    public RecordAppendResult append(String ttName,
                                     long timestamp,
                                     byte[] key,
                                     byte[] value,
                                     Header[] headers,
        ...
        Deque<ProducerBatch> batches = transientTopicInfoMap.computeIfAbsent(ttName, t -> new ArrayDeque<>());
        ...
        try {
            synchronized (batches) {
                RecordAppendResult appendResult = tryAppend(timestamp, key, value, headers, callbacks, batches, nowMs);
                if (appendResult != null) return appendResult;
            }

            if (buffer == null) {
                byte maxUsableMagic = apiVersions.maxUsableProduceMagic(); // TODO: change it to Transient Topic Produce Version
                ...
                log.trace("Allocating a new {} byte message buffer for transient topic {} with remaining timeout {}ms", size, ttName, maxTimeToBlock);
            ...
            synchronized (batches) {
                RecordAppendResult appendResult = appendNewBatch(ttName, batches, timestamp, key, value, headers, callbacks, buffer, nowMs);
     ...
     *
     * @param ttName The name of transient topic
     * @param dq The queue
     * @param timestamp The timestamp of the record
     * @param key The key for the record
     * @param value The value for the record
     ...
     * @param nowMs The current time, in milliseconds
     */
    private RecordAppendResult appendNewBatch(String ttName,
        ...
        ProducerBatch batch = new ProducerBatch(new TopicPartition(ttName, 0), recordsBuilder, nowMs); // TODO: make it Transient Topic Producer Batch?
    ...
    private boolean isMuted(String ttName) {
        return muted.contains(ttName);
    }
    ...
    public List<ProducerBatch> expiredBatches(long now) {
        ...
        for (Deque<ProducerBatch> deque : transientTopicInfoMap.values()) {
    ...
    public void reenqueue(ProducerBatch batch, long now) {
        ...
        Deque<ProducerBatch> deque = getOrCreateDeque(batch.topicPartition.topic());
        synchronized (deque) {
    ...
    public int splitAndReenqueue(ProducerBatch bigBatch) {
        ...
        Deque<ProducerBatch> partitionDequeue = getOrCreateDeque(bigBatch.topicPartition.topic());
        while (!dq.isEmpty()) {
     ...
     * partitions into the set of ready nodes.  If partition has no leader, add the transient topic to the set
     ...
     * @param ttName                The name of transient topic
     * @param deque                 The batch queue for transient topic
     * @param nextReadyCheckDelayMs The delay for next check
     * @param readyNodes            The set of ready nodes (to be filled in)
     * @return The delay for next check
     */
    private long transientTopicReady(Node leader, OptionalInt leaderEpoch, long nowMs,
                                     String ttName, Deque<ProducerBatch> deque,
        ...
        synchronized (deque) {
            ...
            // TODO: ttp don't have to change leader. Check it.
        ...
        if (leader != null) {
            nextReadyCheckDelayMs = batchReady(exhausted, ttName, leader, waitedTimeMs, backingOff,
    ...
    public ReadyCheckResult ready(MetadataSnapshot metadataSnapshot, long nowMs) {
        ...
        TopicPartition physicalTransientTopic = new TopicPartition(Topic.TRANSIENT_TOPIC_NAME, 0);
        Node leader = metadataSnapshot.cluster().leaderFor(physicalTransientTopic);
        OptionalInt leaderEpoch = metadataSnapshot.leaderEpochFor(physicalTransientTopic);
        if (leader == null) {
            unknownLeaderTopics.add(Topic.TRANSIENT_TOPIC_NAME);
        }
        for (Map.Entry<String, Deque<ProducerBatch>> topicInfoEntry : transientTopicInfoMap.entrySet()) {
            final String ttName = topicInfoEntry.getKey();
            nextReadyCheckDelayMs = transientTopicReady(leader, leaderEpoch, nowMs, ttName, topicInfoEntry.getValue(), nextReadyCheckDelayMs, readyNodes);
        }
    ...
    private List<ProducerBatch> drainTransientTopicBatches(MetadataSnapshot metadataSnapshot, int maxSize, long now) {
        int size = 0;
        List<ProducerBatch> ready = new ArrayList<>();
        for (Map.Entry<String, Deque<ProducerBatch>> entry: transientTopicInfoMap.entrySet()) {
            String ttName = entry.getKey();
            if (isMuted(ttName)) continue;
            ...
            synchronized (deque) {
                ...
                if (size + first.estimatedSizeInBytes() > maxSize && !ready.isEmpty()) {
                    ...
                    TopicPartition tp = new TopicPartition(ttName, 0);
                ...
                if (producerIdAndEpoch != null && !batch.hasSequence()) {
                    ...
                    log.debug("Assigned producerId {} and producerEpoch {} to batch with base sequence " +
                            "{} being sent to transient topic {}", producerIdAndEpoch.producerId,
                        producerIdAndEpoch.epoch, batch.baseSequence(), ttName);
    ...
    public Map<Integer, List<ProducerBatch>> drain(MetadataSnapshot metadataSnapshot, Set<Node> nodes, int maxSize, long now) {
        ...
        batches.put(nodes.iterator().next().id(), drainTransientTopicBatches(metadataSnapshot, maxSize, now));
        return batches;
    }
    ...
    public long nextExpiryTimeMs() {
      ...
      /* Visible for testing */
    public Deque<ProducerBatch> getDeque(String ttName) {
        return transientTopicInfoMap.get(ttName);
    }

    /**
     * Get the deque for the given transient topic, creating it if necessary.
     */
    private Deque<ProducerBatch> getOrCreateDeque(String ttName) {
        return transientTopicInfoMap.computeIfAbsent(ttName, ig -> new ArrayDeque<>());
    }
    ...
    public void abortIncompleteBatches() {
        ...
        abortBatches();
        this.transientTopicInfoMap.clear();
    }
    ...
    void abortBatches(final RuntimeException reason) {
        for (ProducerBatch batch : incomplete.copyAll()) {
            Deque<ProducerBatch> dq = getDeque(batch.topicPartition.topic());
            synchronized (dq) {
                batch.abortRecordAppends();
                dq.remove(batch);
            }
    ...
    public void muteTransientTopic(String ttName) {
        muted.add(ttName);
    }

    public void unmuteTransientTopic(String ttName) {
        muted.remove(ttName);
    }
