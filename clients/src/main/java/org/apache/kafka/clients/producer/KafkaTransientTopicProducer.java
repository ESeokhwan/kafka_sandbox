...
import org.apache.kafka.clients.producer.internals.TransientTopicProducerSender;
import org.apache.kafka.clients.producer.internals.TransientTopicRecordAccumulator;
 ...
 * </p>
 */
public class KafkaTransientTopicProducer<K, V> implements Producer<K, V> {

    private final Logger log;
    private static final String JMX_PREFIX = "kafka.transienttopic.producer";
    public static final String NETWORK_THREAD_PREFIX = "kafka-transient-topic-producer-network-thread";
    public static final String PRODUCER_METRIC_GROUP_NAME = "transient-topic-producer-metrics";
    ...
    private final TransientTopicProducerSender sender;
     ...
     *
     */
    public KafkaTransientTopicProducer(final Map<String, Object> configs) {
     ...
     */
    public KafkaTransientTopicProducer(Map<String, Object> configs, Serializer<K> keySerializer, Serializer<V> valueSerializer) {
     ...
     */
    public KafkaTransientTopicProducer(Properties properties) {
     ...
     *                         be called in the producer when the serializer is passed in directly.
     */
    public KafkaTransientTopicProducer(Properties properties, Serializer<K> keySerializer, Serializer<V> valueSerializer) {
        this(Utils.propsToMap(properties), keySerializer, valueSerializer);
    }

    // visible for testing
    @SuppressWarnings({"unchecked", "this-escape"})
    KafkaTransientTopicProducer(ProducerConfig config,
        ...
        try {
            ...
            if (transactionalId == null)
                logContext = new LogContext(String.format("[Transient Topic Producer clientId=%s] ", clientId));
            else
                logContext = new LogContext(String.format("[Transient Topic Producer clientId=%s, transactionalId=%s] ", clientId, transactionalId));
            log = logContext.logger(KafkaTransientTopicProducer.class);
            ...
            this.accumulator = new TransientTopicRecordAccumulator(logContext,
                    batchSize,
                    compression,
                    lingerMs(config),
                    retryBackoffMs,
    ...
    KafkaTransientTopicProducer(ProducerConfig config,
                                ...
                                TransientTopicProducerSender sender,
                                ProducerInterceptors<K, V> interceptors,
                                Time time,
        ...
        this.time = time;
        this.clientId = config.getString(ProducerConfig.CLIENT_ID_CONFIG);
        this.log = logContext.logger(KafkaTransientTopicProducer.class);
    ...
    // visible for testing
    TransientTopicProducerSender newSender(LogContext logContext, KafkaClient kafkaClient, ProducerMetadata metadata) {
        int maxInflightRequests = producerConfig.getInt(ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION);
        int requestTimeoutMs = producerConfig.getInt(ProducerConfig.REQUEST_TIMEOUT_MS_CONFIG);
        ProducerMetrics metricsRegistry = new ProducerMetrics(this.metrics);
        Sensor throttleTimeSensor = TransientTopicProducerSender.throttleTimeSensor(metricsRegistry.senderMetrics);
        KafkaClient client = kafkaClient != null ? kafkaClient : ClientUtils.createNetworkClient(producerConfig,
                this.metrics,
                "transient-topic-producer",
                logContext,
                apiVersions,
                time,
                maxInflightRequests,
        ...
        return new TransientTopicProducerSender(logContext,
                client,
                metadata,
                this.accumulator,
     ...
     * @throws AuthorizationException error indicating that the configured
     ...
     * {@link AuthorizationException}, then the only option left is to call {@link #close()}.
     ...
     * {@link AuthorizationException} are considered fatal errors. However,
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
            ...
            try {
                clusterAndWaitTime = waitOnMetadata(nowMs, maxBlockTimeMs);
            } catch (KafkaException e) {
            ...
            TransientTopicRecordAccumulator.RecordAppendResult result = accumulator.append(record.topic(), timestamp, serializedKey,
                    serializedValue, headers, appendCallbacks, remainingWaitMs, nowMs, cluster);
            assert appendCallbacks.getPartition() != RecordMetadata.UNKNOWN_PARTITION; // TODO: check it whether should remove it or not
     ...
     */
    private ClusterAndWaitTime waitOnMetadata(long nowMs, long maxWaitMs) throws InterruptedException {
        String transientTopic = Topic.TRANSIENT_TOPIC_NAME;
        Cluster cluster = metadata.fetch();

        // add topic to metadata topic list if it is not there already and reset expiry
        metadata.add(Topic.TRANSIENT_TOPIC_NAME, nowMs);

        Integer partitionsCount = cluster.partitionCountForTopic(transientTopic);
        ...
        do {
            log.trace("Requesting metadata update for topic {}.", transientTopic);
            metadata.add(transientTopic, nowMs + elapsed);
            int version = metadata.requestUpdateForTopic(transientTopic);
            sender.wakeup();
            try {
                metadata.awaitUpdate(version, remainingWaitMs);
            } catch (TimeoutException ex) {
                // Rethrow with original maxWaitMs to prevent logging exception with remainingWaitMs
                final String errorMessage = getErrorMessage(transientTopic, maxWaitMs);
                if (metadata.getError(transientTopic) != null) {
                    throw new TimeoutException(errorMessage, metadata.getError(transientTopic).exception());
                }
            ...
            if (elapsed >= maxWaitMs) {
                final String errorMessage = getErrorMessage(transientTopic, maxWaitMs);
                if (metadata.getError(transientTopic) != null && metadata.getError(transientTopic).exception() instanceof RetriableException) {
                    throw new TimeoutException(errorMessage, metadata.getError(transientTopic).exception());
                }
                throw new TimeoutException(errorMessage);
            }
            metadata.maybeThrowExceptionForTopic(transientTopic);
            remainingWaitMs = maxWaitMs - elapsed;
            partitionsCount = cluster.partitionCountForTopic(transientTopic);
    ...
    private String getErrorMessage(String topic, long maxWaitMs) {
        return String.format("Topic %s not present in metadata after %d ms.", topic, maxWaitMs);
    }
    ...
    public List<PartitionInfo> partitionsFor(String topic) {
        ...
        try {
            return waitOnMetadata(time.milliseconds(), maxBlockTimeMs).cluster.partitionsForTopic(topic);
        } catch (InterruptedException e) {
            throw new InterruptException(e);
        }
    }
     ...
     *  - partition callback
     */
    private class AppendCallbacks implements TransientTopicRecordAccumulator.AppendCallbacks {
