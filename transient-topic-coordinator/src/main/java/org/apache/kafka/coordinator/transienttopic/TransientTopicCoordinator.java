...
public class TransientTopicCoordinator {
    ...
    public CompletableFuture<TransientTopic> createNewTransientTopic(
        ...
        CompletableFuture<TransientTopic> responseFuture = new CompletableFuture<>();
        runtime.scheduleWriteOperation(
            "transient-topic-creation",
            topicPartitionFor(topicId),
            ...
            if (!responseFuture.isDone()) {
                responseFuture.complete(new TransientTopic(topicId, topicName, new TransientTopicPartition("AAA", 0, 0)));
            }
            return null;
        });
